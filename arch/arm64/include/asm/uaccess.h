...
#define __put_mem_asm(store, reg, x, addr, label, type)			\
	...
	: : "rZ" (x), "r" (addr) : : label)
#else
/* 6.1 backport: upstream made asm-goto unconditional, but 6.1 still
 * supports compilers without asm-goto-output, so keep the err-based
 * fallback (same shape as the __get_mem_asm fallback above). */
#define __put_mem_asm(store, reg, x, addr, label, type) do {		\
	...
	: "+r" (__pma_err)						\
	: "r" (x), "r" (addr));						\
	if (__pma_err) goto label; } while (0)
#endif
