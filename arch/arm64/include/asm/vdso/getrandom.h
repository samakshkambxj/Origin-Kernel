...
static __always_inline const struct vdso_rng_data *__arch_get_vdso_rng_data(void)
{
	...
	if (IS_ENABLED(CONFIG_TIME_NS) && _vdso_data->clock_mode == VDSO_CLOCKMODE_TIMENS)
		/* 6.1 backport: upstream uses CONFIG_PAGE_SHIFT (newer
		 * kernels); PAGE_SHIFT from asm/page.h is equivalent. */
		return (void *)&_vdso_rng_data + VVAR_TIMENS_PAGE_OFFSET * (1UL << PAGE_SHIFT);
