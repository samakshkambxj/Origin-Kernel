...
#undef PAGE_SIZE
#undef PAGE_MASK
/* 6.1 backport: upstream uses CONFIG_PAGE_SHIFT, a generic Kconfig symbol
 * that only exists in newer kernels; 6.1 arm64 provides PAGE_SHIFT from
 * asm/page.h (backed by CONFIG_ARM64_PAGE_SHIFT), which is equivalent. */
		       ...
		       unsigned int flags, void *opaque_state, size_t opaque_len)
{
	...
	if (unlikely(opaque_len == ~0UL && !buffer && !len && !flags)) {
		...
		/*
		 * Upstream uses MAP_DROPPABLE | MAP_ANONYMOUS here, but
		 * MAP_DROPPABLE does not exist on 6.1, so report plain
		 * anonymous memory. This is strictly safe: without
		 * droppable pages the state can never be zeroed under
		 * memory pressure, and the straddle/generation checks
		 * below remain valid.
		 */
		params->mmap_flags = MAP_ANONYMOUS;
